...
def open_windows_choise():
    ...
    def update_name_entry(event, column_cb, name_entry):
        ...
        print(f"Selected column: {selected_column}")  # Débogage pour vérifier la sélection
        if selected_column:
            ...
            print(f"Updated name_entry: {name_entry.get()}")  # Débogage pour vérifier la mise à jour
    ...
    def add_condition():
        ...
        # Simplification : Affichage d'un message quand une option est sélectionnée
        def combobox_selected(event, cb=column_cb):
            print(f"Combobox selected: {cb.get()}")
            update_name_entry(event, cb, name_entry)

        # Associer les widgets à la vérification
        name_entry.bind("<<ComboboxSelected>>", combobox_selected)
