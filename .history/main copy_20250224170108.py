...
def open_windows_choise():
    ...
    def update_name_entry(event, column_cb, name_entry):
        ...
        if selected_column:
            ...
            name_entry.insert(0, selected_column + "_")  # Ajouter le nom de la colonne avec "_"
            print(f"Nom mis à jour dans name_entry: {name_entry.get()}")  # Débogage
    def test_update_name_entry():
        name_entry.delete(0, tk.END)  # Effacer le contenu précédent
        name_entry.insert(0, 'Test_')  # Ajouter une valeur statique
        print(f"Nom de test mis à jour: {name_entry.get()}")
    ...
    def add_condition():
        ...
        test_button = tk.Button(window, text="Test Update", command=test_update_name_entry)
        test_button.pack(pady=5)
        # Liaison de l'événement de sélection dans column_cb
        def combobox_selected(event):
            print(f"Combobox sélectionnée, valeur: {column_cb.get()}")  # Débogage
            update_name_entry(event, column_cb, name_entry)

        column_cb.bind("<<ComboboxSelected>>", combobox_selected)
