...
def open_windows_choise():
    ...
    def validate_condition():
        ...
        nonlocal add_button, send_button  # Permet d'utiliser les boutons définis plus bas
        ...
        column, name, operator, value = conditions[-1]
        ...
        is_valid = (
            ...
            value.get().strip() != "" and value.get().replace(
                ".", "", 1).isdigit()  # Valeur numérique
        )
    ...
    def update_name_entry(event, column_cb, name_entry):
        ...
        if selected_column:
            ...
            name_entry.insert(0, selected_column + "_")
        validate_condition()
    ...
    def add_condition():
        ...
        column_cb = ttk.Combobox(
            row_frame, values=numeric_columns, state="readonly")
        ...
        operator_cb = ttk.Combobox(
            row_frame, values=operators, state="readonly")
        ...
        column_cb.bind("<<ComboboxSelected>>", lambda event: update_name_entry(
            event, column_cb, name_entry))
        column_cb.bind("<<ComboboxSelected>>",
                       lambda event: validate_condition())
        name_entry.bind("<KeyRelease>", lambda event: validate_condition())
        operator_cb.bind("<<ComboboxSelected>>",
                         lambda event: validate_condition())
    ...
    # Bouton Ajouter (désactivé au départ)
    add_button = tk.Button(window, text="Ajouter",
                           command=add_condition, state=tk.DISABLED)
    add_button.pack(pady=5)

    # Bouton Envoyer (désactivé au départ)
    send_button = tk.Button(window, text="Envoyer", state=tk.DISABLED)
