...
def open_windows_choise():
    ...
    def update_name_entry(event, column_cb, name_entry, operator_cb):
        """Met à jour name_entry en fonction de la colonne et de l'opérateur sélectionnés."""
        selected_column = column_cb.get()
        selected_operator = operator_cb.get()

        # Mapping des opérateurs aux suffixes
        

        suffix = operator_suffixes.get(selected_operator, "")
        
        if selected_column and suffix:
            name_entry.delete(0, tk.END)
            name_entry.insert(0, f"{selected_column}_{suffix}")
